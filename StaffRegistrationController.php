...
class StaffRegistrationController extends Controller {
    ...
    public function actionCreate() {
        $model = new StaffRegistration;
        $la_staffPassword = YII::app()->params['staffPassword'];
        ...
        if (isset($_POST['StaffRegistration'])) {
            ...
            if (StaffRegistration::model()->checkUserEmail($model->email) == 0) {
                ...
                if ($model->save()) {
                    ...
                    $modelUser->password = md5($la_staffPassword);
...
//        ));
    ...
    public function actionExportStaffDetails() {
         ...
         * 
         */
        echo $ls_sql = "SELECT  SR.*, GROUP_CONCAT(distinct JT.job_type) jobTypeName, GROUP_CONCAT(distinct WA.area_name) areaName
    ...
    public function actionSendSMSOrMail($id, $email, $mobile) {
        ...
        if ($email != "") {
            ...
            foreach ($commandForReminder AS $lv_reminder) {
                ...
                $ls_reminder.= 'at <b>' . $lv_reminder["hospital_unit"] . '</b> on <b>' . Utility::changeDateToUK($lv_reminder["shift_start_datetime"]) . '</b> to <b>' . Utility::changeDateToUK($lv_reminder["shift_end_datetime"]) . '</b><br>';
            }
            $data['email'] = $email;
            ...
            foreach ($commandForReminder AS $lv_reminder) {
                ...
                if ($ls_reminder != "")
                    $ls_reminder.= ', %0a';
                $ls_reminder.= 'at ' . $lv_reminder["hospital_unit"] . ' on ' . Utility::changeDateToUK($lv_reminder["shift_start_datetime"]) . ' to ' . Utility::changeDateToUK($lv_reminder["shift_end_datetime"]);
            }
            $data['mobile'] = $mobile;
            $data['reminder'] = $ls_reminder;
    ...
    public function actionSendAllSMSOrMail($action) {
        ...
        if ($action == "mail") {
            $sqlQueryForReminder = "SELECT u.first_name, u.mobile, u.email, h.hospital_unit, s.shift_start_datetime, s.shift_end_datetime FROM {{user}} u, {{shift_management_for_hospital}} s, {{booking}} b, {{hospital_unit}} h, {{staff_registration}} sr WHERE h.hospital_unit_id = s.hospital_unit_id AND u.id = b.staff_id AND s.staff_request_id = b.staff_request_id AND u.staff_id = sr.staff_id AND b.cancel_by_whom = 0 AND sr.staff_status = 'A' AND s.shift_start_datetime BETWEEN '" . $ls_startDate . "' AND '" . $ls_endDate . "' ORDER BY s.shift_start_datetime";
            $commandForReminder = Yii::app()->db->createCommand($sqlQueryForReminder)->queryAll();

            $sqlQueryForReminderEmail = "SELECT u.email FROM {{user}} u, {{shift_management_for_hospital}} s, {{booking}} b, {{hospital_unit}} h, {{staff_registration}} sr WHERE h.hospital_unit_id = s.hospital_unit_id AND u.id = b.staff_id AND s.staff_request_id = b.staff_request_id AND u.staff_id = sr.staff_id AND sr.staff_status = 'A' AND b.cancel_by_whom = 0 AND s.shift_start_datetime BETWEEN '" . $ls_startDate . "' AND '" . $ls_endDate . "' GROUP BY u.email ORDER BY s.shift_start_datetime";
            ...
            $sqlQueryForReminder = "SELECT u.first_name, u.email, h.hospital_unit, s.shift_start_datetime, s.shift_end_datetime FROM {{user}} u, {{shift_management_for_hospital}} s, {{booking}} b, {{hospital_unit}} h, {{staff_registration}} sr WHERE h.hospital_unit_id = s.hospital_unit_id AND u.id = b.staff_id AND s.staff_request_id = b.staff_request_id AND u.staff_id = sr.staff_id AND sr.staff_status = 'A' AND b.cancel_by_whom = 0 AND s.shift_start_datetime BETWEEN '" . $ls_startDate . "' AND '" . $ls_endDate . "' ORDER BY s.shift_start_datetime";
            $commandForReminder = Yii::app()->db->createCommand($sqlQueryForReminder)->queryAll();
