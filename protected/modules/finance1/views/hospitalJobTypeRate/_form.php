...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
		<?php echo $form->labelEx($model,'hospital_unit_id'); ?>
                                    ...
                                    if (!$model->isNewRecord) {
                                            $getName = HospitalUnit::model()->getHospitalName($model->hospital_unit_id); 
                                            $model->hospital_unit_id = $getName;
                                    }elseif ($model->hospital_unit_id != "") {
                                            $getName = HospitalUnit::model()->getHospitalName($model->hospital_unit_id); 
                                            $model->hospital_unit_id = $getName;
                                    }
                                    ?>
	...
	<div class="row">
		<?php echo $form->labelEx($model,'finance_job_type_id'); ?>
                                    ...
                                    <?php if (!$model->isNewRecord) 
                                        echo CHtml::dropDownList('HospitalJobTypeRate[finance_job_type_id]', $selectedJobType, $allJobType, array('single'=>'single'));?>
...
?>
    <script>
        $(function ()
        {
            $("#HospitalJobTypeRate_hospital_unit_id").autocomplete({
                source: 'index.php?r=admin/default/autocompleteHospitalUnitName'
            });
        })
