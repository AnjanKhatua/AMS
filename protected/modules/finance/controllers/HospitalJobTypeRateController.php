...
//            $model->attributes = $_POST['HospitalJobTypeRate'];
            ...
            $i = 0;
            $j = 0;
            if ((count($post['HospitalJobTypeRate']['hospital_unit_id']) != 0) && ($post['HospitalJobTypeRate']['pay_rate_for_staffs'] != "") && ($post['HospitalJobTypeRate']['rate'] != "") && (count($post['HospitalJobTypeRate']['finance_job_type_id']) != 0)) {
                for ($i = 0; $i < count($post['HospitalJobTypeRate']['hospital_unit_id']); $i++) {
                    for ($j = 0; $j < count($post['HospitalJobTypeRate']['finance_job_type_id']); $j++) {
                        $model = new HospitalJobTypeRate;

                        $model->pay_rate_for_staffs = $post['HospitalJobTypeRate']['pay_rate_for_staffs'];
                        $model->rate = $post['HospitalJobTypeRate']['rate'];
                        $model->finance_job_type_id = $post['HospitalJobTypeRate']['finance_job_type_id'][$j];
                        $model->hospital_unit_id = $post['HospitalJobTypeRate']['hospital_unit_id'][$i];
...
//                $model->hospital_unit_id = $command['hospital_unit_id'];
                if (count($_POST['HospitalJobTypeRate']['hospital_unit_id']) != 0)
                    $model->hospital_unit_id = $_POST['HospitalJobTypeRate']['hospital_unit_id'][0];
                ...
                $model->pay_rate_for_staffs = $_POST['HospitalJobTypeRate']['pay_rate_for_staffs'];
                $model->save();
                $model->finance_job_type_id = $_POST['HospitalJobTypeRate']['finance_job_type_id'];
                $model->hospital_unit_id = $_POST['HospitalJobTypeRate']['hospital_unit_id'];
            }
    ...
    public function actionGetJobTypeForHospital() {
        $sqlQueryForJobType = "SELECT j.id, j.job_type_name FROM {{job_type_for_finance}} j, {{hospital_job_type_rate_map}} hm WHERE j.id = hm.finance_job_type_id AND hm.hospital_unit_id = '" . $_POST['hospital_unit_id'] . "'";
        ...
        foreach ($commandForJobType as $k => $lv_jobType) {
            $allJobType .=
                    '<option value="' . $lv_jobType['id'] . '">' . $lv_jobType['job_type_name'] . '</option>';
        }
        echo $allJobType;
    }
