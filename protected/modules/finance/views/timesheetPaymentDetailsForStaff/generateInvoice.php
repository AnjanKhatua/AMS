...
$ls_sqlForPaymentList = "SELECT t.invoice_date FROM {{timesheet_payment_details_for_staff}} t, {{timesheet_training_deduction_week}} td WHERE t.invoice_date = td.invoice_date AND td.apply_status = 'N' GROUP BY t.invoice_date";
...
?>
<h3>View payment list for <?php echo CHtml::dropDownList('invoice_date', $_REQUEST['id'], $la_forPaymentList); ?><input type="hidden" id="invoice_date" value="<?php echo $model->invoice_date; ?>"></h3>
<?php
...
echo CHtml::button('Export Staff Invoice', array(
    'submit' => array('TimesheetPaymentDetailsForStaff/ExportGenerateInvoiceBeforeDeduction'),
    'confirm' => 'Are you sure to Export Invoice?'
));
?>

<?php
echo CHtml::button('Generate Invoice', array(
    'submit' => array('TimesheetPaymentDetailsForStaff/trainingDeduction'),
    'confirm' => 'Are you sure to Generate Invoice?'
));
?>
<?php $this->endWidget(); ?>
